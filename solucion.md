el primer error que noto son que no tiene cerrado los server.listen

el segundo error que noto es que un dash había un guión el cual debería de ser un guión, en la linea 15 "Content-Type": "application-json" estaba asi y deberia estar asi, "Content-Type": "application/json" ,}
...
const texto = await fs.readFile(filePath, "utf-8") res.end(JSON.stringify(JSON.parse(texto))) aqui necesitaba parsear para luego volverlo stringify
